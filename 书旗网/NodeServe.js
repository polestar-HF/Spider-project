...
// node 服务参考：https://github.com/DingZaiHub/PythonSpider/blob/master/19-BOSS%E7%9B%B4%E8%81%98/index.js
var express = require('express')
var app = express()
var decode = require('./decode_text')  // 导入decode_text模块，并命名为decode
var bodyParser = require('body-parser');  // 导入请求体解析器
// 调整参数大小限制，否则会提示参数过大。
app.use(bodyParser.urlencoded({limit: '50mb', extended: true}));

// 开启decode_text路由
app.post('/decode_text', function (req, res) {
    // 获取请求的真实IP
    ...
    // 获取POST请求的formdata
    let result = req.body;
    let content = result.content; // 接收 python 传的参数
    let seed = result.seed;
    let ts = result.ts;

    // 调用decode模块中的parseText方法，该方法需要提前module.exports导出
    var text = decode.parseText(content);
