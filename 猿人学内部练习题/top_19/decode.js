...
const {_0x1d3c} = require('./decrpytStr');
// 插件2(...)
parserCallExpression = {
    CallExpression(path){
        let {callee, arguments} = path.node;

        if(!types.isIdentifier(callee, {"name": "_0x1d3c"}) || arguments.length !== 2){
            return
        }
        if (!arguments.every((val) => types.isStringLiteral(val))){
            return;
        }
...
traverse(ast, parserCallExpression, opts= {});
