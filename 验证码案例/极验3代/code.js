...
function get_l(setLeft, track, new_challenge, c, s, gt_str) {
    // setLeft 滑动距离
    // challenge get.php请求返回
    // track 滑块轨迹数组
    // c、s get.php请求返回
    // gt_str get.php请求返回 --> 定值
    r = this
    ...
    var distance = track[track.length - 1][0]
    // 滑块滑动过程总时长, 可以直接取滑块轨迹的最后一组的t [x, y, t]
    var passtime = track[track.length - 1][2]

    var aa = _BBCA(_GEy(track), c, s);
    var o = {
        "lang": 'zh-cn', // 语言 --> 固定
        "userresponse": H(distance, new_challenge), // 通过滑块滑动距离计算得到
        "passtime": passtime, // 滑块滑动过程时长
        "imgload": 38, // 验证码图片加载时间  --> 30 - 80 ms
        "aa": aa, // 滑块轨迹加密生成
        "ep": {}, // 鼠标事件记录
    };
    o[$_CAGDp(727)] = U(gt_str + new_challenge["slice"](0, 32) + o[$_CAGEe(716)]);
    return V[$_CAGEe(342)](gt[$_CAGEe(209)](o), _CBFG());
}

function get_w(setLeft, new_challenge, track_str, c, s, gt_str) {
    // setLeft 滑动距离
    // challenge get.php请求返回
    // track 滑块轨迹数组
    // c、s get.php请求返回
    ...
    var $_CAGCw = ["$_CAGGF"].concat($_CAGDp);
    var $_CAGEe = $_CAGCw[1];
    var slide_track = JSON.parse(track_str)
    // var slide_track = track_str

    var u = _CBEE();
    var l = get_l(setLeft, slide_track, new_challenge, c, s, gt_str)
...
// // 滑动距离
// setLeft = 126
// new_challenge = "979522cbf85181a7e41ac7b19b1f2e1cex"
// var slide_track = [
//     [
//         -21,
//         -35,
...
// s = '59715348'
// gt_str = "019924a82c70bb123aae90d483087f94"
//
// result = get_w(setLeft, new_challenge, slide_track, c, s, gt_str)
// console.log(result)