...
"""
响应数据的img数据数量与网页上显示数量不符, 响应数据img标签到含有class属性
class的值是通过下面代码生成的，通过加密的md5值即可确定那些img标签不会在网页显示
"""
# var j_key = '.' + hex_md5(btoa(data.key + data.value).replace(/=/g, ''));
# $(j_key).css('display', 'none');


def btoa(content) -> str:
    return base64.b64encode(content.encode()).decode()


def hex_md5(content) -> str:
    return hashlib.md5(content.encode()).hexdigest()


def get_num_image():
    """
    :return: 不显示的图片class与响应数据
    """
    headers = {
        ...
        'cookie': 'Hm_lvt_c99546cf032aaa5a679230de9a95c7db=1633658631; qpfccr=true; no-alert3=true; Hm_lvt_9bcbda9cbf86757998a2339a0437208e=1633658635; tk=-6696083641513350383; sessionid=tlj6f02quuw6fmvilqwv2xopq7zvrdnt; Hm_lpvt_9bcbda9cbf86757998a2339a0437208e=1633658679; Hm_lpvt_c99546cf032aaa5a679230de9a95c7db=1633659480',
    }
    response = requests.get(f'https://match.yuanrenxue.com/api/match/4?page={page}', headers=headers).json()
    content = btoa(response.get('key') + response.get('value')).replace('=', '')
    return hex_md5(content), response.get('info')


def parse_data(response):
    """
    筛选要显示的图片组，难点在于数字位置还原
    css位置还原参考: https://www.jianshu.com/p/a41e09fc0420
    :return: 排序后的结果
    """

    def array_sort(array: list):
        # css位置还原, 使用css值除以11（单个数字图片宽度：11px）得出位置 + 当前索引 = 新的位置
        result_list = array.copy()  # 排序后的数据
        for i, img_tup in enumerate(array):
            index = int(img_tup[1] / 11)
            if index != 0:
                result_list[i + index] = array[i]
        return result_list

    no_display_key, image_list = response
    img_list = etree.HTML(image_list)
    for data in img_list.xpath('//td'):
        mk = []
        for img in data.xpath('./img'):
            img_class = img.xpath('./@class')[0].split(' ')[-1]
            if img_class != no_display_key:
                style = float(img.xpath('./@style')[0].split(':')[-1].replace('px', ''))
                image = number_map.get(img.xpath('./@src')[0])
                mk.append((image, style))
        number_list = array_sort(mk)
        yield int(''.join([str(result[0]) for result in number_list]))
